...
| B04 | **Priorisation automatique Eisenhower** — calcul urgence × importance → priorité affichée sur les tâches | Haute | Moyenne | Task, B02 | Quelle est la règle précise : seuils d'urgence en jours ? Qui peut modifier l'importance (uniquement Resp. GdP, Resp. Ops, DG) ? ⚠️ **Limitation POC** : urgence calculée sur échéance seule, sans tenir compte de la durée de la tâche (`Task.duration`). Affiner en Phase 3. |
| B05 | **Détection automatique des conflits de planning et de charge** — intra-projet, inter-projets, tâches annexes | Haute | Très haute | Task, Timesheet, Employee, B01, B02 | Le blocage est-il dur (impossible de sauvegarder) ou doux (alerte + confirmation) ? Quel seuil : 100 % de charge ou configurable ? ⚠️ **Limitation POC** : détection basée sur `_assign` uniquement — les tâches non assignées via ce champ ne sont pas vérifiées. Affiner en Phase 3 avec `Project User`. `LIMIT 10` sur les requêtes SQL — seuls les 10 premiers conflits sont affichés dans le message d'erreur. |
...
| B23 | **Alertes congé vs tâche existante** — notification au manager et chef de projet lors de la validation d'un congé en conflit | Haute | Faible | Leave Application (app `hrms` requise), Task, Email Notification | ⚠️ **En attente décision RH InovaYa** : ERPNext HR natif (`hrms`) vs connecteur RH externe (Lucca, Payfit). Code disponible sur branche `POC_InovaYa_V16`, hook `on_submit` Leave Application prêt. Si outil externe : remplacer le hook par une API webhook entrante. `Task.assigned_to` absent en v16 — remplacé par `Task._assign` (JSON) + `Project User`. `Project.project_manager` absent — remplacé par `Project.owner`. |
