...
Rapport Script Report "Tableau de Bord Collaborateur InovaYa"
...
def _build_date_conditions(from_date, to_date, date_prefix=""):
    ...
    date_prefix : '' pour Task (DATE() wrap) ou '' pour Annexe Task (type Date natif).
    """
    clauses = []
    params  = {}
    if from_date:
...
def get_data(filters):
    ...
    # Jointure tabProject pour afficher le nom lisible (project_name), pas PROJ-xxxx
    task_conds  = [
        "t.is_template = 0",
    ...
    # Clauses date avec préfixe t. et DATE() car exp_start/end_date sont DateTime sur tabTask
    task_date_conds = []
    if from_date:
    ...
    task_rows = frappe.db.sql(f"""
        SELECT
            ...
            t.status,
            COALESCE(t.inovaya_eisenhower_info, '')             AS eisenhower,
            COALESCE(NULLIF(t.inovaya_hours_per_day, 0), 8.0)  AS hours_per_day,
    ...
    if status_filter and status_filter != "All":
        # Statuts Annexe Task : Open / Working / Done
        # "Pending Review" (Task uniquement) ne retourne rien → comportement correct
    ...
    annexe_rows = frappe.db.sql(f"""
        SELECT
            ...
            ''                                              AS project,
            status,
            COALESCE(inovaya_eisenhower_info, '')           AS eisenhower,
    ...
    for r in list(task_rows) + list(annexe_rows):
        ...
        # Heures réalisées : 0 pour Annexe Tasks (pas de lien Timesheet natif — POC)
        actual = ts_by_task.get(r.task_name, 0.0) if r.source == "Task" else 0.0

        # Retard : échéance dépassée ET pas terminé
        is_late = bool(
            end
            and end < today_d
        ...
        rows.append({
            ...
            "exp_end_date":    r.exp_end_date,   # type Date → Frappe formate
            "is_late":         1 if is_late else 0,
            # Champ hors colonne — accessible dans le formatter JS via data.is_annexe
            # Permet d'afficher "—" au lieu de "0.0" pour les Annexe Tasks
            # (pas de lien Timesheet natif sur ce DocType au POC)
            "is_annexe":       1 if r.source == "Annexe Task" else 0,
        })

    # ── [5] Tri : retard d'abord, puis échéance ASC ───────────────────────────
    rows.sort(key=lambda r: (
        0 if r["is_late"] else 1,                          # retard en tête
        str(r["exp_end_date"] or "9999-12-31"),            # échéance croissante
    ))
