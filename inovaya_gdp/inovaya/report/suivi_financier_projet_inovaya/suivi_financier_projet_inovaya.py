...
Sources :
  Prévu    → SUM(amount_planned)  tabBudget Detaille InovaYa  (B15)
...
def _date_clauses(filters):
    ...
    # po_clause/pi_clause sont construits en dur à partir de valeurs fixes
    # (pas de données utilisateur interpolées) — pas de risque injection SQL.
    # Les valeurs de date sont passées via params (paramètres liés MariaDB).
    """
...
def get_data(filters):
    ...
    # po_clause injecté via f-string mais construit en dur — pas de risque injection SQL.
    ...
    # pi_clause injecté via f-string mais construit en dur — pas de risque injection SQL.
    ...
    for row in budget_rows:
        ...
        data.append({
            ...
            "amount_engaged":  None,   # Pas de ventilation PO par catégorie (Phase 3)
            "amount_actual":   None,   # Idem PI
            "variance":        None,
            "pct_consumed":    None,
            "is_total":        0,
        })
