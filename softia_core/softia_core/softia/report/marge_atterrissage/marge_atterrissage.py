"""
B21 — Rapport "Marge Atterrissage"
Calcul de marge prévisionnelle vs marge projetée (atterrissage).

Colonnes : Poste | Montant (€)

Lignes :
  CA projet           = SUM(Jalon Facturation.amount)
  Budget initial      = SUM(Budget Detaille Projet.amount_planned)
  Dépenses réalisées  = SUM(Purchase Order.grand_total) docstatus=1
  Coût ETP réalisé    = SUM(Timesheet Detail.hours) × softia_default_hourly_rate
  Reste à faire (EAC) = Project.softia_eac
  ─────────────────────────────────────────────────────────────────
...
def get_data(filters):
    ...
    hourly_rate = float(
        frappe.db.get_single_value("System Settings", "softia_default_hourly_rate") or 50
    )
    ...
    eac = float(frappe.db.get_value("Project", project, "softia_eac") or 0)
